...
export default function CollabScreen() {
  ...
  const loadData = useCallback(async () => {
    setLoading(true);
    const [p, props] = await Promise.all([fetchCurrentProfile(), fetchProposals('pending')]);
    setCurrentProfile(p);
    setProposals(props);
    setLoading(false);
  }, []);
  ...
  if (loading) {
    return (
      <SafeAreaView className="flex-1 items-center justify-center bg-[#fcFAF8] dark:bg-slate-950">
    ...
    <SafeAreaView className="flex-1 bg-[#fcFAF8] dark:bg-slate-950" edges={['top']}>
      ...
      <ScrollView
        ...
        <View className="mb-8 mt-2 px-6">
          <View className="w-full flex-row items-center justify-between rounded-[24px] border border-[#e5fad8] bg-[#f1fdec] px-6 py-8 dark:border-emerald-950/20 dark:bg-emerald-950/10">
            <View>
              <Text className="text-[42px] font-black leading-[48px] text-gray-900 dark:text-white">
                {proposals.length}
              </Text>
              <Text className="mt-1 text-[14px] font-medium text-gray-500 dark:text-slate-400">
                Pending proposals
              </Text>
            </View>
              ...
              {proposals.length === 0 ? (
                <View className="mt-4 items-center justify-center py-10">
                  <Feather
                    ...
                    className="mb-4"
                  />
                  <Text className="text-[14px] font-medium text-[#94a3b8] dark:text-slate-600">
                    No pending reviews
                  </Text>
                </View>
              ) : (
                proposals.map((prop) => {
                  ...
                  return (
                    <View
                      key={prop.id}
                      className="mb-4 rounded-[24px] border border-gray-100 bg-white p-5 shadow-sm dark:border-slate-800 dark:bg-slate-900">
                      {/* Proposer Header */}
                      <View className="mb-5 flex-row items-center justify-between">
                        <View className="flex-1 flex-row items-center">
                          <View className="mr-3 h-10 w-10 overflow-hidden rounded-full bg-gray-100 dark:bg-slate-800">
                            {proposer?.avatar_url ? (
                              <Image
                                source={{ uri: proposer.avatar_url }}
                          ...
                          <View className="flex-1 pr-2">
                            ...
                            <Text
                              className="mt-0.5 text-[13px] text-gray-500 dark:text-slate-400"
                              numberOfLines={1}>
                              Proposed changes for{' '}
                              <Text className="font-bold text-[#32CD32]">
                                {target?.full_name ?? 'Member'}
                              </Text>
                            </Text>
                          </View>
                        </View>
                      ...
                      {/* Data Box */}
                      <View className="mb-4 rounded-[16px] bg-[#f8f9fa] p-4 dark:bg-slate-950">
                        {prop.change_type === 'profile_update' ? (
                            ...
                            {Object.keys(data).map((key) => {
                              ...
                              return (
                                <View key={key} className="mb-3">
                                  <Text className="mb-1 text-[10px] font-bold text-gray-400 dark:text-slate-500">
                                    {label}
                                  </Text>
                                  <View className="flex-row items-center justify-between">
                                    <Text
                                      className="mr-2 flex-1 text-[13px] text-gray-400 dark:text-slate-600"
                                      numberOfLines={1}>
                                      {String(original?.[key] ?? 'None')}
                                    </Text>
                                    <Feather
                                      name="arrow-right"
                                      size={12}
                                    ...
                                    />
                                    <Text
                                      className="flex-1 text-right text-[13px] font-bold text-[#32CD32]"
                                      numberOfLines={1}>
                                      {String(val)}
                                    </Text>
                                  </View>
                          ...
                          <View>
                            <Text className="mb-1 text-center text-[10px] font-bold text-gray-400 dark:text-slate-500">
                              NEW TIMELINE EVENT
                            </Text>
                            <View className="items-center">
                              ...
                              <Text className="text-[13px] font-semibold text-[#32CD32]">
                                {data.year}
                              </Text>
                              {data.description ? (
                                <Text
                                  className="mt-1 text-center text-[13px] text-gray-500 dark:text-slate-400"
                                  numberOfLines={2}>
                                  {data.description}
                                </Text>
                              ) : null}
                          ...
                          <View>
                            <Text className="mb-1 text-center text-[10px] font-bold text-red-400 dark:text-red-900">
                              DELETE TIMELINE EVENT
                            </Text>
                            <View className="items-center">
                              <Text className="text-[16px] font-bold text-gray-400 line-through dark:text-slate-600">
                                {data.title}
                              </Text>
                              <Text className="text-[13px] font-semibold text-gray-400 dark:text-slate-600">
                                {data.year}
                              </Text>
                            </View>
                          </View>
                          ...
                          <View className="items-center">
                            <Text className="mb-2 text-[10px] font-bold text-gray-400 dark:text-slate-500">
                              NEW PROFILE PHOTO
                            </Text>
                            <View className="h-20 w-20 overflow-hidden rounded-full border-2 border-[#e5fad8] bg-gray-100 dark:border-emerald-950/20 dark:bg-slate-800">
                      ...
                      <View className="flex-row gap-x-3">
                        <TouchableOpacity
                          ...
                          className="flex-1 flex-row items-center justify-center rounded-full bg-[#111827] py-[14px] dark:bg-white">
                          {processingId === prop.id ? (
                            ...
                            <Text className="text-[14px] font-bold text-white dark:text-black">
                              Approve
                            </Text>
                          )}
                        </TouchableOpacity>
                        <TouchableOpacity
                          ...
                          disabled={!!processingId}
                          className="flex-1 items-center justify-center rounded-full bg-[#f3f4f6] py-[14px] dark:bg-slate-800">
